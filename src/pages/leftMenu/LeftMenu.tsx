  ...
  const rederItem = (route: RouteObj) => (
    ...
    <Menu.Item 
            ...
            icon={route.icon ? <img src={route.icon} alt='icon'></img>  : undefined}>
              {
              !collapsed?
              <Text style={{marginLeft: route.icon || route.isRoot? 0 : 30, color: activeKey() === route.routeProps?.path as string? '#000' :route.color}}>{ route.label}</Text>
              :  <Text></Text>}
              
              </Menu.Item>
    :<Menu.SubMenu key={route.routeProps? (route.routeProps.path as string) : undefined} 
    icon={route.icon ? <SVG src={route.icon} $filled={ activeGroupKey() === route.routeProps?.path } $colorText={route.color ?? '#000'}></SVG> : undefined}
    title={route.label}>
      {route.items && route.items.map(child => (
          <Menu.Item 
            ...
            icon={child.icon ? <img src={child.icon} alt='icon'></img>  : undefined}>
              {
              !collapsed?
              <Text style={{marginLeft: child.icon || child.isRoot? 0 : 30, color: activeKey() === child.routeProps?.path as string? '#000' :child.color}}>{ child.label}</Text>
              :  <Text style={{color: activeKey() === child.routeProps?.path as string? '#000' :child.color}}>{ child.label}</Text>}
